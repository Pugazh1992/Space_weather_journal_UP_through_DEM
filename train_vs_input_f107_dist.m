...
Purpose
================================================================================
This MATLAB diagnostic script compares the distribution of the F10.7 solar-radio
flux index used in the neural-network/AETHER-P3 training input data with a
lognormal uncertain-input distribution generated for Monte-Carlo-style
space-weather uncertainty studies.
...
For each case, the script:
    ...
    - propagates the uncertain F10.7 input through a nonlinear test system,
      y = 5*x^2 + 3,
...
Figures produced in MATLAB:
    Figure 1:
        Comparative histograms of NN training F10.7 data and uncertain
        lognormal F10.7 input samples for the three Swarm-C cases.

    Figure 2:
        Comparison of Monte-Carlo F10.7 input histograms against the UT sigma
        points for the same lognormal input distributions.

    Figure 3:
        Comparison of nonlinear-system outputs generated from the MC F10.7
        samples and the corresponding UT sigma-point outputs.
...
Command-window tables/diagnostics:
    ...
    - tblSpaceWeather:
        Case metadata for the selected Swarm-C F10.7 scenarios.

    - input_SW_stats:
        MC statistics of the uncertain F10.7 input samples.

    - ut_SW_stats:
        UT-reconstructed statistics of the uncertain F10.7 input.
    ...
    - ovl_coeff:
        Kernel-density overlap coefficient between the training F10.7
        distribution and each uncertain input distribution.
...
Std          = [f107_stats.std_f107_train; ap_stats.std_ap_train];
Skewness     = [f107_stats.skew_f107_train; ap_stats.skew_ap_train];
Minimum      = [f107_stats.min_f107_train; ap_stats.min_ap_train];
Maximum      = [f107_stats.max_f107_train; ap_stats.max_ap_train];
...
sgtitle(sprintf(['Comparitive histograms of the NN training $F_{10.7}$ data and' ...
    ' uncertain input $F_{10.7}$ (lognormal) with a noise ratio of %0.2f)'], ...
     noise_ratio), 'Interpreter','latex', 'FontSize',15);


fig2 = figure('Color','w','Name','MC - UT comparison (1x3)');
tiledlayout(1,3,'TileSpacing','compact','Padding','compact');
sgtitle(sprintf(['Comparision of the uncertain lognormal input $F_{10.7}$ histogram (MC)' ...
    ' and the sigma points (UT) with a noise ratio of %0.2f)'], ...
     noise_ratio), 'Interpreter','latex', 'FontSize',15);

fig3 = figure('Color','w','Name','MC - UT output comparison (1x3)');
tiledlayout(1,3,'TileSpacing','compact','Padding','compact');
% sgtitle(sprintf(['Comparision of the linear system output using uncertain lognormal input $F_{10.7}$ histogram (MC)' ...
%     ' and the sigma points (UT) with a noise ratio of %0.2f)'], ...
%      noise_ratio), 'Interpreter','latex', 'FontSize',15);
sgtitle(sprintf(['Comparision of the nonlinear system output using uncertain lognormal input $F_{10.7}$ histogram (MC)' ...
    ' and the sigma points (UT) with a noise ratio of %0.2f)'], ...
     noise_ratio), 'Interpreter','latex', 'FontSize',15);
...
for jj = 1:length(noise_ratio)
    ...
    % for ii = 1:1
        ...
        % xdata1_mc_lognormal(:, 12) = generate_log10_lognormal_samples(mean_linear, variance_linear, ...
        %     n_samples_MC);  
        xdata1_mc_lognormal(:, 12) = generate_log10_lognormal_samples(mean_linear, variance_linear, ...
            n_samples_MC, 'Verbose', false);        

        % Extract the F10.7 values from the training data:
        input_f107 = xdata1_mc_lognormal(:, 12);     

        % Compute the first three statistical moments of F10.7:
        input_f107_stats{ii}.mean_f107_train = mean(input_f107);
        input_f107_stats{ii}.std_f107_train = std(input_f107);
        input_f107_stats{ii}.skew_f107_train = skewness(input_f107);
        
        % Compute the extreme values of F10.7:
        input_f107_stats{ii}.min_f107_train = min(input_f107);
        input_f107_stats{ii}.max_f107_train = max(input_f107);
        input_f107_stats{ii}.range_f107_train = input_f107_stats{ii}.max_f107_train - ...
        input_f107_stats{ii}.min_f107_train;

        %%% 1️⃣ Histogram of histogram of training and case(ii) F10.7
        figure(fig1);
        nexttile;
        histogram(train_f107, nbins, 'Normalization','percentage', ...
            'FaceColor','blue', 'EdgeColor','none', 'FaceAlpha',0.5);
        % histogram(train_f107, nbins, 'Normalization','percentage', ...
        %     'FaceColor','blue', 'EdgeColor','none', 'FaceAlpha',0.5);        
        grid on;
        hold on;
        xline(f107_stats.mean_f107_train, '--b', 'LineWidth',1);
        xline(f107_stats.min_f107_train, '-b', 'LineWidth',1);
        xline(f107_stats.max_f107_train, '-b', 'LineWidth',1);
        histogram(input_f107, nbins, 'Normalization','percentage', ...
            'FaceColor','red', 'EdgeColor','none', 'FaceAlpha',0.5);
        xline(input_f107_stats{ii}.mean_f107_train, '--r', 'LineWidth',1);
        xline(input_f107_stats{ii}.min_f107_train, '-r', 'LineWidth',1);
        xline(input_f107_stats{ii}.max_f107_train, '-r', 'LineWidth',1);        
        ...
        ovl_coeff(ii) = kde_overlap(train_f107, input_f107)
        ...
        ut_mean = Wm.' * xSigma;
        ut_f107_stats{ii}.mean_f107_train = ut_mean;   % mean uses Wm
        ut_var  = sum( Wc .* (xSigma - ut_mean).^2 );  % variance uses Wc
        ut_f107_stats{ii}.std_f107_train = sqrt(ut_var);
        ut_mu3  = sum( Wm .* (xSigma - ut_mean).^3 );  % third central (use Wm)
        ut_f107_stats{ii}.skew_f107_train = ut_mu3 / (ut_var^(3/2));  
        ...
        % output_mc = lin_system(input_f107, lin_sys_coeff_a, lin_sys_coeff_b);
        % 
        ...
        output_mc = non_lin_system(input_f107, nonlin_sys_coeff_a, nonlin_sys_coeff_b);
        ...
        output_mc_f107_stats{ii}.mean_f107 = mean(output_mc);
        output_mc_f107_stats{ii}.std_f107 = std(output_mc);
        output_mc_f107_stats{ii}.skew_f107 = skewness(output_mc);
        output_mc_f107_stats{ii}.min_f107 = min(output_mc);
        output_mc_f107_stats{ii}.max_f107 = max(output_mc);        

        % Output statistical moments from sigma points:
        ut_mean_output = Wm.' * output_ut;
        output_ut_f107_stats{ii}.mean_f107 = ut_mean_output;   % mean uses Wm
        ut_var_output  = sum( Wc .* (output_ut - ut_mean_output).^2 );  % variance uses Wc
        output_ut_f107_stats{ii}.std_f107 = sqrt(ut_var_output);
        ut_mu3_output  = sum( Wm .* (output_ut - ut_mean_output).^3 );  % third central (use Wm)
        output_ut_f107_stats{ii}.skew_f107 = ut_mu3_output / (ut_var_output^(3/2));         
        ...
        grid on;
        hold on;
        xline(output_mc_f107_stats{ii}.mean_f107, '--r', 'LineWidth',1);
        xline(output_mc_f107_stats{ii}.min_f107, '-r', 'LineWidth',1);
        xline(output_mc_f107_stats{ii}.max_f107, '-r', 'LineWidth',1); 
...
% Build the table with the desired content
SW_index     = ["F107"; "F107"; "F107"];
Mean         = [input_f107_stats{1}.mean_f107_train; input_f107_stats{2}.mean_f107_train;...
    input_f107_stats{3}.mean_f107_train;];
Std          = [input_f107_stats{1}.std_f107_train; input_f107_stats{2}.std_f107_train;...
    input_f107_stats{3}.std_f107_train;];
Skewness     = [input_f107_stats{1}.skew_f107_train; input_f107_stats{2}.skew_f107_train;...
    input_f107_stats{3}.skew_f107_train;];
Minimum      = [input_f107_stats{1}.min_f107_train; input_f107_stats{2}.min_f107_train;...
    input_f107_stats{3}.min_f107_train;];
Maximum      = [input_f107_stats{1}.max_f107_train; input_f107_stats{2}.max_f107_train;...
    input_f107_stats{3}.max_f107_train;];

Mean_ut         = [ut_f107_stats{1}.mean_f107_train; ut_f107_stats{2}.mean_f107_train;...
    ut_f107_stats{3}.mean_f107_train;];
Std_ut          = [ut_f107_stats{1}.std_f107_train; ut_f107_stats{2}.std_f107_train;...
    ut_f107_stats{3}.std_f107_train;];
Skewness_ut     = [ut_f107_stats{1}.skew_f107_train; ut_f107_stats{2}.skew_f107_train;...
    ut_f107_stats{3}.skew_f107_train;];
...
% For the MC samples:
Mean_mc         = [output_mc_f107_stats{1}.mean_f107; output_mc_f107_stats{2}.mean_f107;...
    output_mc_f107_stats{3}.mean_f107];
Std_mc          = [output_mc_f107_stats{1}.std_f107; output_mc_f107_stats{2}.std_f107;...
    output_mc_f107_stats{3}.std_f107];
Skewness_mc     = [output_mc_f107_stats{1}.skew_f107; output_mc_f107_stats{2}.skew_f107;...
    output_mc_f107_stats{3}.skew_f107];
output_mc_stats = table(Report_case, SW_index, Mean_mc, Std_mc, Skewness_mc);

% For the UT sigma points:
Mean_ut         = [output_ut_f107_stats{1}.mean_f107; output_ut_f107_stats{2}.mean_f107;...
    output_ut_f107_stats{3}.mean_f107];
Std_ut          = [output_ut_f107_stats{1}.std_f107; output_ut_f107_stats{2}.std_f107;...
    output_ut_f107_stats{3}.std_f107];
Skewness_ut     = [output_ut_f107_stats{1}.skew_f107; output_ut_f107_stats{2}.skew_f107;...
    output_ut_f107_stats{3}.skew_f107];
