...
Purpose:
    ...
    density neural network for Swarm-C test cases in which the Ap geomagnetic
    index is treated as the uncertain space-weather input. The script loads
    previously generated Monte-Carlo (MC) and Unscented Transform (UT) input
    .mat files, normalizes the 65-dimensional input vectors using the
    training-data statistics, runs the trained neural-network model, and saves
    the density-prediction and uncertainty outputs as MATLAB-readable .mat
    files.

    The uncertainty in Ap is assumed to have already been sampled or represented
    by sigma points using a base-10 lognormal distribution in the companion
    MATLAB input-generation script sd_swarmc_ap_lognormal_mc_ut_v1.m. This
    script is therefore the model evaluation / density-nowcasting stage of the
    workflow, not the input-sample generation stage.
...
Input files required:
    For each Ap case and noise-ratio folder, the script expects the following
    MATLAB v7.3 input files generated by sd_swarmc_ap_lognormal_mc_ut_v1.m:
...
Output produced: figure or table or .mat files:
    ...
    Tables / command-window output:
        - Prints the Swarm-C Ap case table.
    ...
    MATLAB .mat files:
        ...
        Output/Swarm/<High_Ap | Low_Ap | Medium_Ap>/
