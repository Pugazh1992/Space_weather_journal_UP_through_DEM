...
Swarm-C test case.  The script treats the Ap geomagnetic index as the single
uncertain space-weather input and keeps the remaining model input features
deterministic.
...
For each selected Swarm-C case and noise ratio, the script:
  ...
  3. Replaces the Ap column with base-10 lognormal MC samples.
  4. Constructs three UT sigma points for the same base-10 lognormal Ap
     uncertainty model.
  5. Replaces the Ap column in the UT input matrix with these sigma points.
  6. Generates a histogram showing the MC Ap distribution and the UT sigma
     point locations.
...
only one scalar random variable, Ap, is being perturbed.
...
Figures:
  ...
  - Each figure shows the base-10 lognormal MC Ap input distribution and
    overlays the three UT sigma-point locations.
  - With the active setup, the script generates 6 figures:
        3 Ap cases x 2 noise ratios.
