...
Purpose
===============================================================================
This MATLAB post-processing script analyzes the density-prediction results
obtained from Ruochen's global thermosphere-density neural network for the
Swarm-C Ap lognormal uncertainty study.  Among the 65 neural-network input
features, only the Ap geomagnetic index is treated as a random variable; all
remaining features are kept deterministic for each selected case.

The script loads the MC and UT input files produced by
sd_swarmc_ap_lognormal_mc_ut_v1.m and the corresponding neural-network outputs
produced by mt_swarmc_ap_lognormal_mc_ut_v1.py.  It then converts the
normalized/logarithmic neural-network outputs back to the physical density
domain, combines aleatoric and epistemic uncertainty, compares consolidated MC
statistics against recombined UT statistics, and includes a baseline case
without Ap input uncertainty.

The active configuration post-processes the following Swarm-C Ap cases:
    1. High_Ap
    2. Low_Ap
    3. Medium_Ap

The active noise-ratio setting is:
    noise_ratio = 0.25
...
MATLAB/toolbox dependencies:
    - MATLAB base functionality for loading data, array operations, tables,
      figures, tiled layouts, histograms, legends, and command-window output.
    - Statistics and Machine Learning Toolbox functions, including makedist,
      lognrnd, and related distribution/statistical utilities.
    - Parallel Computing Toolbox is recommended because the physical-density
      conversion loops use parfor.

Project/data dependencies:
    - Output/Model/Model1/GPoutput.mat
    - Output/Swarm/<Ap case>/Noise_ratio_<ratio>/lognormal_mc_ut/
    - MC and UT input .mat files generated by the Ap input-generation script.
    - MC and UT neural-network result .mat files generated by the Ap
      Python model-execution script.

===============================================================================
Input files required
===============================================================================
Normalization/reference output file:
    Output/Model/Model1/GPoutput.mat

For each Ap case, where the case is one of
High_Ap, Low_Ap, or Medium_Ap, and for each active noise ratio, the script
expects the following files in
Output/Swarm/<case>/Noise_ratio_<noise_ratio>/lognormal_mc_ut/:

    Noisy_ap_MC_1000000.mat
        Contains the MC input matrix xdata1_mc_lognormal and the corresponding
        output vector ydata1_mc.

    sigma_points_ap_ut_3.mat
        Contains the UT input matrix xdata1_ut, output vector ydata1_ut, and
        UT recombination weights Wm and Wc.

    mc_results.mat
        Contains the neural-network prediction structure mc_results for the
        MC input ensemble.

    ut_results.mat
        Contains the neural-network prediction structure ut_results for the
        UT sigma-point inputs.

===============================================================================
Output produced: figures, tables, and .mat files
===============================================================================
Figures produced:
    - For each active Ap case and noise ratio, the script creates a MATLAB
      figure named 'MC Results Histograms (2x3)' with a 2-by-2 layout:
          1. Histogram of the lognormal Ap MC input distribution with the
             three UT sigma-point locations overlaid.
          2. Histogram of the predicted normalized/logarithmic density mean,
             mu_hat_z.
          3. Histogram of the predicted normalized/logarithmic density standard
             deviation, sigma_hat_z.
          4. Consolidated physical-density distribution comparing MC, UT, and
             baseline mean and one-sigma intervals.

Tables/printed outputs produced:
    - Command-window display of the Swarm-C case table tblSpaceWeather.
    - Command-window summaries for each case/noise ratio, including:
          * consolidated MC mean density in physical space,
          * baseline mean density in physical space,
          * UT mean density in physical space,
          * relative error between UT and MC mean density,
          * consolidated MC density standard deviation,
          * baseline density standard deviation,
          * UT density standard deviation,
          * relative error between UT and MC density standard deviation.

.mat files produced:
    - This script does not save a new .mat file by default.  It loads existing
      input/result .mat files and generates MATLAB figures plus command-window
      diagnostics.
...
Research context: Propagation of lognormal Ap input uncertainty through a
global thermosphere-density neural network using MC and UT methods.
Documentation note: Header comments expanded for repository documentation and
reproducibility.
===============================================================================
%}
