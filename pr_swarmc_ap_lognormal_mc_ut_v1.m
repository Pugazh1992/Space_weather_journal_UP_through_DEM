...
simulations. Out of the 65 different inputs, only Ap is considered as a
random variable with lognormal distribution. Both MC & UT simulation use
the lognormal distributions

version 1:
 - Three different cases of Ap are considered with a single satellite:
 Swarm C.
 - The number of Monte-Carlo samples is set to 1 million and the number of
 draws for the consolidated MC is set to 100 for each mean and standard
 ...
 for density.
- input Ap data is shown along with the UT sigma points.
...
% Index of Ap in the input data to the NN:
sw_index = 16;
...
% Suitable table name
tblSpaceWeather = table(Report_case, Satellite, ap_indication);
...
for jj = 1:length(noise_ratio)
    ...
    % for ii = 1:1
        ...
        child_dir_str = strcat(char(tblSpaceWeather.ap_indication(ii)), ...
        ...
        % Obtain the mean of the predicted density in logarithmic space:
        mc_results.pred_density_after_log = mc_results.mu_1*Ystd + Ymean;
    
        % Obtain the total standard deviation of the predicted density in logarithmic space:
       ...
       ut_results_using_lognormal.density_var_physical = ut_results_using_lognormal.density_std_physical.^2;
        ...
        title('Histogram of input lognormal $A_p$ and its UT sigma points','Interpreter','latex');
        ax = gca;
        ...
        if isequal(display_result_space, 'logarithmic')
            ...
            histogram(mc_results_using_lognormal.all_samples, nbins, 'Normalization','percentage', ...
                'FaceColor',[0.2 0.45 0.85], 'EdgeColor','none', 'FaceAlpha',0.85);
            grid on; hold on;
            % set(gca, 'XScale', 'log');  % To display the x-axis in log-scale
        ...
        % grid on
        % hold on
        % xlabel(sprintf('$A_p$ index'),'Interpreter','latex');
        ...
        %     tblSpaceWeather.Report_case(ii), tblSpaceWeather.ap_indication(ii), ...
...
function [all_samples, mu_hat, sigma_hat] = draw_from_many_gaussians_loop_v3(mu_vec, sig_vec, m)
%DRAW_FROM_MANY_GAUSSIANS_LOOP_V3
% Sample m times from each of n Gaussians (with their own mean and std),
% stack all samples into one vector, and return consolidated statistics.
%
% Syntax
%   [all_samples, mu_hat, sigma_hat] = draw_from_many_gaussians_loop_v3(mu_vec, sig_vec, m)
%
% Inputs
%   mu_vec : [n x 1] or [1 x n] vector of means for each Gaussian
%   sig_vec: [n x 1] or [1 x n] vector of standard deviations (>= 0)
%   m      : positive integer, number of samples to draw per Gaussian
%
% Outputs
...
%
% Notes
% - This function intentionally uses a FOR-LOOP to (1) draw m samples per
%   Gaussian and (2) stack them in order. This preserves the "m per
%   distribution" bookkeeping (helpful for later per-source analyses).
% - The stacked distribution is a mixture of Gaussians (generally NOT
%   Gaussian). mu_hat and sigma_hat are empirical statistics of that mixture.

    % -------------------------------
    % 0) Input checks and preparation
    % -------------------------------
    if nargin < 3
        error('Provide mu_vec, sig_vec, and m (samples per Gaussian).');
    end

    mu_vec  = mu_vec(:);      % ensure column
    sig_vec = sig_vec(:);     % ensure column
    n = numel(mu_vec);

    if numel(sig_vec) ~= n
        error('Lengths of mu_vec and sig_vec must match.');
    end
    if ~all(isfinite(mu_vec))
        error('All means must be finite.');
    end
    if ~all(isfinite(sig_vec)) || any(sig_vec < 0)
        error('All standard deviations must be finite and nonnegative.');
    end
    ...
    % ------------------------------------------
    all_samples = zeros(m*n, 1);  % preallocate
    for i = 1:n
        mu_i  = mu_vec(i);
        sig_i = sig_vec(i);

        % Draw m samples from N(mu_i, sig_i^2)
        % (mvnrnd handles scalar variance; equivalent to mu_i + sig_i*randn(m,1))
