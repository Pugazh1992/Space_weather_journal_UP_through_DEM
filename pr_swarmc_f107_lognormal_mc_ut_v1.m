...
Swarm-C F10.7 lognormal uncertainty study.  Among the 65 neural-network input
features, only the F10.7 index is treated as a random variable; all remaining
features are kept deterministic for each selected case.

The script loads the MC and UT input files produced by
sd_swarmc_f107_lognormal_mc_ut_v1.m and the corresponding neural-network
outputs produced by mt_swarmc_f107_lognormal_mc_ut_v1.py.  It then converts the
...
without F10.7 input uncertainty.

The active configuration post-processes the following Swarm-C F10.7 cases:
    1. High_f107
    2. Low_f107
    3. Medium_f107
...
Project/data dependencies:
    ...
    - MC and UT input .mat files generated by the F10.7 input-generation script.
    - MC and UT neural-network result .mat files generated by the F10.7
...
For each F10.7 case, where the case is one of
High_f107, Low_f107, or Medium_f107, and for each active noise ratio, the script
...
===============================================================================
Figures produced:
    - For each active F10.7 case and noise ratio, the script creates a MATLAB
      figure named 'MC Results Histograms (2x3)' with a 2-by-2 layout:
          1. Histogram of the lognormal F10.7 MC input distribution with the
...
Research context: Propagation of lognormal F10.7 input uncertainty through a
